...
import {ActivityIndicator, Linking, RefreshControl, SafeAreaView, ScrollView, Text, View} from 'react-native';
    ...
    return (
        <SafeAreaView style={{flex: 1, backgroundColor: COLORS.lightWhite}}>
            ...
            <>
                ...
                {!isLoading && data && data.length > 0 && data[0].session_state === "active" &&
                    <StartSession item={data[0]}/>}
                {!isLoading && data && data.length > 0 && data[0].session_state === "in_progress" && data[0].progress_tasks === "false" &&
                    <CompleteTasks item={data[0]}/>}
                {!isLoading && data && data.length > 0 && data[0].session_state === "in_progress" && data[0].progress_tasks === "true" && data[0].progress_notes === "false" &&
                    <CompleteVisitNotes item={data[0]}/>}
                {!isLoading && data && data.length > 0 && data[0].session_state === "in_progress" && data[0].progress_tasks === "true" && data[0].progress_notes === "true" &&
                    <EndSession item={data[0]}/>}
            </>
        </SafeAreaView>
    );
};
